...
} from "../socket/orderTrackingSocket.js";
...
router.get("/summary", async (req, res) => {
  try {
    ...
    // Get database counts
    const [totalUsersInDB, totalRestaurantsInDB, totalDeliveryPartnersInDB] =
      await Promise.all([
    ...
    const summary = {
      ...
      liveConnections: {
        totalConnected:
          deviceStats.totalConnected +
          orderTrackingUsers +
          orderTrackingPartners,
...
router.get("/live-count", (req, res) => {
  try {
    ...
    const liveCount = {
      totalConnected:
        deviceStats.totalConnected + orderTrackingUsers + orderTrackingPartners,
